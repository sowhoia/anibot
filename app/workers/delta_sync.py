...
async def delta_sync(updated_at_from: str | None = None, concurrency: int = 3) -> None:
    setup_logging(level=settings.log_level, log_path=settings.log_file)
    client = KodikClient()
    ingest = IngestService(get_session())
    ts = updated_at_from or (datetime.utcnow() - timedelta(days=1)).isoformat()

    raw_items = await client.fetch_delta(updated_at_from=ts)
    if not raw_items:
        logger.info("No updates since %s", ts)
        return
    ...
    logger.info("Delta sync done. Updated items: %s", len(raw_items))
